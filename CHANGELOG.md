...
- Fixed the inventory writer aborting the build when a heading has an empty anchor id (e.g. an empty `#` heading, or a misparsed table that collapses into an empty heading node). `InventoryItem` requires a non-empty `name`, so this threw `ArgumentError: name must have non-zero length`. The inventory entry is now skipped with a warning; the page still renders [#375](https://github.com/LuxDL/DocumenterVitepress.jl/issues/375)
